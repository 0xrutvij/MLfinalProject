...
class neuralNetwork:
  ...
  def __init__(self, input, output,numOfHidden,actChoice,eta):
    ...
    self.hiddenlayer = pd.DataFrame((np.random.rand(self.numOfFeatures,self.numOfHidden))-0.5)/20
    self.outputlayer = pd.DataFrame(np.random.rand(self.numOfHidden,1))-0.5
