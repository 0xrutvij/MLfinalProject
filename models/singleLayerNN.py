...
class neuralNetwork:
  ...
  def __init__(self, input, output,numOfHidden,actChoice):
    ...
    self.hiddenLayerBias = pd.DataFrame(np.ones(input.shape[0])).T
  ...
  def testMyNetwork(self,xtest,ytest):
    ...
    testInput = pd.DataFrame(np.ones(xtest.shape[0])).T.append(xtest.reset_index(drop=True).T,ignore_index=True)
...
def do_backprop(eta,max_epoch,xtrain,ytrain,xtest,ytest):
  ...
  myNetwork = neuralNetwork(xtrain, ytrain,3,1)
  ...
  myNetwork = neuralNetwork(xtrain, ytrain,3,2)
  ...
  myNetwork = neuralNetwork(xtrain, ytrain,3,3)
